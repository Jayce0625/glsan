...
from tools.visualizer import visualize_gt_boxes
...
def crop_add_sr(crop_dataset_dir, sr_dataset_dir, sr_model, dataset_type="visdrone", sr_size=500):
    ...
    for img_i in tqdm(range(len(crop_json['images']))):
        ...
        while anno_ind < len_anno and crop_json['annotations'][anno_ind]['image_id'] == img_info['id']:  # 每次for都会把VisDronePlus中的一张图片的全部标注放到sr_json中（保留超分前图像标注），因为这个方法的超分既会保留未超分的也会保留超分的
            anno = crop_json['annotations'][anno_ind]
            # anno['bbox_id'] = sr_bbox_id  原代码错误
            anno['id'] = sr_bbox_id
            anno['image_id'] = sr_image_id  # 原代码缺失出错，尽管要保留未超分的图像，检测框位置大小类别均不变，但是anno id和image_id都需要改变，因为数据已经增强了所以对应图片索引也变化了
            sr_bbox_id += 1
            anno_ind += 1
        ...
        if ((img_i + 1) % 1000 == 1):
            print("anno_ind_start:", anno_ind_start, ", anno_ind_end:", anno_ind_end, ", crop_image_id:", (img_i + 1), ", sr_image_id: ", sr_image_id)
        sr_image_id += 1
        ...
        is_sr = False
        if (math.sqrt(img_info['height'] * img_info['width']) < sr_size):
        # if (img_info['height'] < sr_size and img_info['width'] < sr_size):
            is_sr = True
            sr_input = torch.FloatTensor(img.copy()).to(device).permute(2, 0, 1).unsqueeze(0)
            img = sr_model(sr_input)
            img = img.squeeze().permute(1, 2, 0).cpu().numpy().astype(np.uint8)
        ...
        if is_sr:
            ...
            if not exists(sr_img_file):
                cv2.imwrite(sr_img_file, img)
        anno_ind = anno_ind_end  # 原代码缺失所以出错
...
if __name__ == "__main__":
    ...
    crop_dataset_dir = join(root, 'VisDronePlus')
    sr_dataset_dir = join(root, 'VisDronePlusSR')
    crop_add_sr(crop_dataset_dir, sr_dataset_dir, sr_model, dataset_type='visdrone')
