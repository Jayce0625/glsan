...
class GlsanNet(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs):
        if not self.training:
            return self.inference(batched_inputs)  # 只有推理才能端到端使用裁剪和超分
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        dirs1 = './visualization/baseline/'
        dirs2 = './visualization/roi/'
        if not os.path.exists(dirs1):
            os.makedirs(dirs1)
        if not os.path.exists(dirs2):
            os.makedirs(dirs2)
        time_now = time.strftime("%Y%m%d-%H", time.localtime())
        baseline_path = os.path.join(dirs1, time_now)
        roi_path = os.path.join(dirs2, time_now)
        ...
        for r_i in range(len(results)):
            ...
            self.visualize_boxes(results[r_i], image.copy(),
                                 os.path.join(baseline_path, batched_inputs[r_i]["file_name"].split('/')[-1]),
        ...
        for i in range(len(batched_inputs)):
            ...
            for img_i in range(len(part_imgs_per_img)):
                ...
                if self.sr:
                    # super-resolution 因为是推理，所以只需要超分图像而不需要annotations
        ...
        for r_i in range(len(merged_results)):
            self.visualize_boxes(merged_results[r_i], images[r_i].copy(),
                                 os.path.join(roi_path, batched_inputs[r_i]["file_name"].split('/')[-1]),
                                 show_score=False, show_class=False)
